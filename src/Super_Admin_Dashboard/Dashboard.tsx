...
const Dashboard: React.FC = () => {
  const { can, loading: permissionsLoading, roleName, refresh } = usePermissions();
  ...
  useEffect(() => {
    const unsubscribe = subscribeBulkImportJobs(setImportJobs);
    return () => {
      unsubscribe();
    };
  }, []);
  ...
  const fetchCurrentUserProfile = async () => {
    try {
      ...
      setTeacherSchool({
        ...
        logoUrl: profile?.schoolLogoUrl || null,
      });
    } catch {
  ...
  const fetchDashboardExamUpdates = async () => {
    ...
    try {
      ...
      setUpcomingExamEvents(schedules.flat().sort((a, b) => new Date(a.examDate).getTime() - new Date(b.examDate).getTime()));
    } catch (error) {
  ...
  const visibleNotices = publishedResults
    ...
    .slice(0, 5);
  const visibleEvents = upcomingExamEvents.filter((event) => event.examDate && getLocalDateStart(event.examDate) >= todayStart).slice(0, 5);

  const handleLogout = () => {
    if (userRole === '2' && teacherAttendanceDirty && !window.confirm('Log out and discard unsaved attendance?')) return;
    setProfileMenuOpen(false);
    localStorage.removeItem('token');
    navigate('/login');
  };

  const handleNavigate = (page: string, type?: 'student' | 'staff', sectionId?: number) => {
    ...
    if (teacherAttendanceDirty && !window.confirm('Leave attendance and discard unsaved changes?')) return;
    setTeacherSection(sectionId);
    setActivePage(page);
  ...
  return (
    <div className="dashboard-wrapper">
      ...
      <Sidebar activePage={activePage} onNavigate={handleNavigate} isCollapsed={isCollapsed} userRole={userRole} attendanceType={attendanceType} schoolName={userRole === '2' ? teacherSchool.schoolName : schools.find((school) => school.id === selectedSchoolId)?.schoolName} schoolLogoUrl={userRole === '2' ? teacherSchool.logoUrl : schools.find((school) => school.id === selectedSchoolId)?.logoUrl} />
      <div className={`dashboard-main ${isCollapsed ? 'sidebar-collapsed' : ''}`}>
        <header className="dashboard-header">
          <div className="header-left">
            <button className="menu-toggle-btn" onClick={() => setIsCollapsed((p) => !p)}>
              <MenuIcon size={24} />
            </button>
            <h1>{activePage}</h1>
            {schools.length > 0 && (
              <select className="school-picker" value={selectedSchoolId || ''} onChange={(e) => setSelectedSchoolId(Number(e.target.value))}>
                {schools.map((school) => (
          ...
          <div className="header-right">
            ...
            {userRole === '2' ? (
              ...
              <>
                ...
                <div className="import-notifications">
                  ...
                  {notificationsOpen && (
                    <div className="import-notification-panel">
            ...
            <div className="profile-menu" ref={profileMenuRef}>
              <button type="button" className="user-avatar" title={userName} aria-label={`${userName} profile menu`} aria-expanded={profileMenuOpen} onClick={() => setProfileMenuOpen((open) => !open)}>
                {userProfilePicture ? <img src={profilePictureUrl(userProfilePicture)} alt={`${userName} profile`} onError={() => setUserProfilePicture(null)} /> : <span>{userInitials}</span>}
              </button>
              {profileMenuOpen && (
        ...
        <ImportResults
          type={selectedImportJob?.type || 'Student'}
          result={
            selectedImportJob
              ? {
                  imported: selectedImportJob.imported,
                  errors: selectedImportJob.errors,
                }
              : null
          }
        ...
        <div className="dashboard-content">
          {permissionsLoading ? (
            <div className="permission-empty">Loading access...</div>
          ) : (
            (() => {
              ...
              if (!isRoleOnlyDashboard && activePermission && !can(activePermission, 'read'))
                return (
                ...
                <>
                  {activePage === 'School List' ? (
                    ...
                    attendanceType === 'student' ? (
                      userRole === '2' ? (
                        <TeacherStudentAttendance initialSection={teacherSection} onDirtyChange={setTeacherAttendanceDirty} />
                      ) : (
                        <StudentAttendance />
                      )
                        ...
                        <div className="chart-section">
                          ...
                          <div className="chart-placeholder">
                            <div className="chart-legend">
                              <span>
                                <span className="legend-dot green"></span>Fee Collection
                              </span>
                              <span>
                                <span className="legend-dot red"></span>Expenses
                              </span>
                            </div>
                            ...
                            <div className="notice-list">
                              {visibleNotices.length === 0 ? (
                                <div className="notice-item">
                                  <div className="notice-title">No published results.</div>
                                </div>
                              ) : (
                            ...
                            <div className="event-list">
                              {visibleEvents.length === 0 ? (
                                <div className="event-item">
                                  <div className="event-title">No upcoming scheduled exams.</div>
                                </div>
                              ) : (
                                visibleEvents.map((event) => (
                                  <div className="event-item" key={event.id}>
                                    <div className="event-title">
                                      {event.examName}: {event.subjectName}
                                    </div>
                                    <div className="event-time">
                                      {event.className} - {event.sectionName} &middot;{' '}
                                      {new Date(event.examDate).toLocaleDateString('en-GB', {
                                        day: '2-digit',
                                        month: 'short',
                                        year: 'numeric',
                                      })}
        ...
        >
          ...
          >
            ...
            >
              ...
              .
              <br />
              Please head over to the Attendance section to mark yourself present or absent.
            </p>
            <button
