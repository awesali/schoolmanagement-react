...
import './InventoryManagement.edit.css';
...
const tabs:Record<Tab,Config>={
 ...
 books:{label:'Books',endpoint:'books',fields:[{key:'bookName',label:'Book Name',required:true},{key:'academicSessionId',label:'Academic Session',type:'number',required:true},{key:'classId',label:'Class Name',type:'number'},{key:'sectionId',label:'Section Name',type:'number'},{key:'subjectId',label:'Subject Name',type:'number'},{key:'publisher',label:'Publisher'},{key:'edition',label:'Edition'},{key:'isbn',label:'ISBN'},{key:'mrp',label:'MRP',type:'number',required:true},{key:'sellingPrice',label:'Selling Price',type:'number',required:true},{key:'discountAmount',label:'Discount',type:'number'}]},
 materials:{label:'Study Materials',endpoint:'products'},
 bookKits:{label:'Book Kits',endpoint:'book-kits',fields:[{key:'kitName',label:'Kit Name',required:true},{key:'academicSessionId',label:'Academic Session',type:'number',required:true},{key:'classId',label:'Class Name',type:'number'},{key:'mrp',label:'MRP',type:'number',required:true},{key:'sellingPrice',label:'Selling Price',type:'number',required:true},{key:'discountAmount',label:'Discount',type:'number'}]},
 uniformKits:{label:'Uniform Kits',endpoint:'uniform-kits',fields:[{key:'kitName',label:'Kit Name',required:true},{key:'academicSessionId',label:'Academic Session',type:'number',required:true},{key:'classId',label:'Class Name',type:'number'},{key:'mrp',label:'MRP',type:'number',required:true},{key:'sellingPrice',label:'Selling Price',type:'number',required:true},{key:'discountAmount',label:'Discount',type:'number'}]},
...
const STUDY_MATERIAL_TABS:Tab[]=['dashboard','books','bookKits','uniformKits'];
...
const InventoryManagement:React.FC<{selectedSchoolId:number|null; mode?:'inventory'|'studyMaterials'}>=({selectedSchoolId,mode='inventory'})=>{
 ...
 const [form,setForm]=useState<Record<string,string>>({}),[open,setOpen]=useState(false),[loading,setLoading]=useState(false),[editingId,setEditingId]=useState<number|null>(null);
 ...
 const save=async(e:React.FormEvent)=>{e.preventDefault();if(!selectedSchoolId||!cfg.fields||!cfg.endpoint)return;const body:any={schoolId:selectedSchoolId,isActive:true};cfg.fields.forEach(f=>body[f.key]=f.type==='number'?Number(form[f.key]||0):(form[f.key]||null));if(tab==='bookKits')body.kitType='Book';if(tab==='uniformKits')body.kitType='Uniform';try{const r=await fetch(`${API_BASE_URL}/api/Inventory/${cfg.endpoint}${editingId?`/${editingId}`:''}`,{method:editingId?'PUT':'POST',headers:headers(),body:JSON.stringify(body)});const text=await r.text();let j:any={};try{j=text?JSON.parse(text):{}}catch{throw new Error(r.ok?'Server returned an invalid response.':text.split('\n')[0]||`Unable to save (${r.status}).`)}if(!r.ok)throw new Error(j.message||`Unable to save (${r.status}).`);setMessage(j.message);setForm({});setEditingId(null);setOpen(false);await load();}catch(e){setMessage(e instanceof Error?e.message:'Unable to save.');}};
 const editRow=(r:any)=>{if(!['books','bookKits','uniformKits'].includes(tab))return;const next:Record<string,string>={};(cfg.fields||[]).forEach(f=>next[f.key]=r[f.key]==null?'':String(r[f.key]));setForm(next);setEditingId(r.id);setOpen(true);window.scrollTo({top:0,behavior:'smooth'});};
 const cols=useMemo(()=>rows.length?Object.keys(rows[0]).filter(x=>!['id','schoolId','productId','academicSessionId'].includes(x)):[],[rows]);
 if(!selectedSchoolId)return <div className="staff-list-loading">Please select a school</div>;
 const cards=mode==='studyMaterials'?[['Total Books',summary.totalBooks],['Book Kits',summary.totalBookKits],['Uniform Kits',summary.totalUniformKits],['Classes Covered',summary.classesCovered],['Average Book Price',`Rs. ${Number(summary.averageBookPrice||0).toLocaleString()}`],['Total Book Value',`Rs. ${Number(summary.totalBookValue||0).toLocaleString()}`],['Total Kit Value',`Rs. ${Number(summary.totalKitValue||0).toLocaleString()}`]]:[['Total Products',summary.totalProducts],['Books',summary.totalBooks],['Uniforms',summary.totalUniforms],['Study Materials',summary.totalStudyMaterials],['Low Stock',summary.lowStockItems],['Today Sales',`Rs. ${Number(summary.todaySales||0).toLocaleString()}`],['Pending Payments',`Rs. ${Number(summary.pendingPayments||0).toLocaleString()}`],['Monthly Revenue',`Rs. ${Number(summary.monthlyRevenue||0).toLocaleString()}`]];
 return <div className="staff-list-container"><div className="staff-list-header"><h2>{mode==='studyMaterials'?'Study Materials':'School Store / Inventory'}</h2></div>
  <div className="management-tabs inventory-tabs" role="tablist">{visibleTabs.map(k=><button key={k} role="tab" type="button" aria-selected={tab===k} className={`management-tab ${tab===k?'active':''}`} onClick={()=>{setRows([]);setEditingId(null);setForm({});setTab(k);setOpen(false);setMessage('');}}>{tabs[k].label}</button>)}</div>
