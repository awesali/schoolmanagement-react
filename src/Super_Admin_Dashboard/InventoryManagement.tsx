...
interface Field { key:string; label:string; type?:string; required?:boolean; optionsKey?:string }
...
const tabs:Record<Tab,Config>={
 ...
 purchases:{label:'Purchase Orders',endpoint:'purchase-orders',fields:[{key:'vendorId',label:'Vendor',type:'select',optionsKey:'vendors',required:true},{key:'purchaseDate',label:'Purchase Date',type:'date',required:true},{key:'invoiceNumber',label:'Invoice Number'},{key:'productId',label:'Product',type:'select',optionsKey:'products',required:true},{key:'quantity',label:'Quantity',type:'number',required:true},{key:'rate',label:'Purchase Rate',type:'number',required:true},{key:'gstPercent',label:'GST %',type:'number'},{key:'discount',label:'Discount',type:'number'}]},
 stock:{label:'Stock Management',endpoint:'stock',postEndpoint:'stock-adjustments',fields:[{key:'productId',label:'Product',type:'select',optionsKey:'products',required:true},{key:'transactionType',label:'Adjustment Type',type:'select',optionsKey:'stockTypes',required:true},{key:'quantity',label:'Quantity',type:'number',required:true},{key:'remarks',label:'Reason / Remarks',required:true}]},
 orders:{label:'Student Orders',endpoint:'orders',fields:[{key:'academicSessionId',label:'Academic Session',type:'select',optionsKey:'sessions',required:true},{key:'studentId',label:'Student',type:'select',optionsKey:'students',required:true},{key:'productId',label:'Product',type:'select',optionsKey:'products',required:true},{key:'quantity',label:'Quantity',type:'number',required:true},{key:'unitPrice',label:'Unit Price',type:'number'},{key:'discount',label:'Discount',type:'number'}]}, issue:{label:'Issue to Students',endpoint:'orders'},
...
const InventoryManagement:React.FC<{selectedSchoolId:number|null; mode?:'inventory'|'studyMaterials'}>=({selectedSchoolId,mode='inventory'})=>{
 ...
 const fieldOptions=(key:string):{value:string;label:string}[]=>{
  ...
  if(optionKey==='students')return lookups.students.filter(x=>x.isActive!==false).map(x=>({value:String(x.id),label:`${x.studentName}${x.rollNumber?` - ${x.rollNumber}`:''}`}));
 ...
 const issueOrder=async(id:number)=>{try{const r=await fetch(`${API_BASE_URL}/api/Inventory/orders/${id}/issue`,{method:'POST',headers:headers()});const j=await r.json();if(!r.ok)throw new Error(j.message||'Unable to issue order.');setMessage(j.message);await Promise.all([load(),loadLookups()]);}catch(e){setMessage(e instanceof Error?e.message:'Unable to issue order.');}};
 ...
 return <div className="staff-list-container"><div className="staff-list-header"><h2>{mode==='studyMaterials'?'Study Materials':'School Store / Inventory'}</h2></div>
  ...
  :<>{cfg.fields&&<div style={{display:'flex',justifyContent:'flex-end',marginBottom:'16px'}}><button className="btn btn-primary" onClick={()=>open?closeForm():setOpen(true)}>{open?'Close':`+ Add ${cfg.label}`}</button></div>}
   {loading?<div className="staff-list-loading">Loading...</div>:rows.length===0?<div className="staff-list-loading">No records found.</div>:<div className="staff-table-wrapper"><table className="staff-table"><thead><tr>{cols.map(c=><th key={c}>{human(c)}</th>)}{tab==='issue'&&<th>Action</th>}</tr></thead><tbody>{rows.map((r,i)=><tr key={r.id??i}>{cols.map((c,columnIndex)=><td key={c}>{EDITABLE_TABS.includes(tab)&&columnIndex===0?<span className="staff-name-link" onClick={()=>startEdit(r)}>{show(r[c])}</span>:show(r[c])}</td>)}{tab==='issue'&&<td>{r.status==='Issued'?<span>Issued</span>:<button type="button" className="btn btn-primary" onClick={()=>issueOrder(r.id)}>Issue</button>}</td>}</tr>)}</tbody></table></div>}</>}
  <Modal isOpen={open} onClose={closeForm} title={`${editingId===null?'Add':'Edit'} ${cfg.label.replace(/s$/,'')}`} submitLabel={editingId===null?'Save':'Update'} formId="inventory-form" showCancel={false} size="large">
   {cfg.fields&&<form id="inventory-form" onSubmit={save} className="form-grid">{cfg.fields.map(f=>{const options=fieldOptions(f.key);return <div className="form-group" key={f.key}><label>{f.label.replace(' ID','')}{f.required?' *':''}</label>{f.type==='select'||options.length>0||['productId','categoryId','academicSessionId','classId','sectionId','subjectId'].includes(f.key)?<select required={f.required} value={form[f.key]||''} disabled={f.key==='sectionId'&&!form.classId} onChange={e=>setForm({...form,[f.key]:e.target.value,...(f.key==='classId'?{sectionId:''}:{})})}><option value="">Select {f.label.replace(' ID','')}</option>{options.map(o=><option key={o.value} value={o.value}>{o.label}</option>)}</select>:<input type={f.type||'text'} step={f.type==='number'?'any':undefined} required={f.required} value={form[f.key]||''} onChange={e=>setForm({...form,[f.key]:e.target.value})}/>}</div>})}</form>}
  </Modal>
 </div>;
};
