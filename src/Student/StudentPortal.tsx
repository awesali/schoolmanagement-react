...
const groups: { title: string; items: Page[] }[] = [
  ...
  { title: 'Academics', items: ['Attendance', 'Exams', 'Results', 'Calendar', 'Events'] },
...
export default function StudentPortal() {
  ...
  return <div className={`sp-layout${dark ? ' sp-dark' : ''}`}>
    ...
    {page === 'Requests' && <>
      ...
      {panel('New request', <form className="sp-action-form" onSubmit={async e => {
        e.preventDefault();
        ...
        <button className="btn btn-primary" disabled={savingAction || !requestForm.recipientUserId}>{savingAction ? 'Submitting...' : 'Submit request'}</button>
      </form>)}
